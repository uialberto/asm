...
namespace Asm.Infra
{
    public abstract class RepositoryIdentity<TEntity> : IDisposable, IRepository<TEntity> where TEntity : class
    {
        private readonly DbSet<TEntity> _dbSet;
        private readonly EfUoWIdentity _unitOfWork;

        public RepositoryIdentity(IUnitOfWork context)
        {
            if (context == null)
                throw new ArgumentNullException(nameof(context));
            if (context is IEfUoW)
                _unitOfWork = context as EfUoWIdentity;
